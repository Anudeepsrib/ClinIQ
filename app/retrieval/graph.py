...
This module defines the complete graph with 5 nodes:

    retrieve → grade_documents → generate → hallucination_check
                    ↑                              |
                    |__ transform_query ←←←←←←←←←←←|
                           (if no docs)       (if hallucinated)

Conditional routing ensures the pipeline retries with query
transformation when documents are irrelevant, and regenerates
when the answer is not grounded in the clinical context.
Retries are capped at ``MAX_QUERY_RETRIES`` (default 3).
"""
...
logger.info("ClinIQ LangGraph compiled with 5 nodes and conditional routing")
